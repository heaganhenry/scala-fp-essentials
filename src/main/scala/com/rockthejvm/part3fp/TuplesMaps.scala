...
object TuplesMaps {
  ...
  val aTuple_v2 = 2 -> "rock the jvm" // INDENTICAL to (2, "rock the jvm")
  ...
  val marysPhoneNumber = phonebook("Mary") // crash with an exception (if no default val given)
  ...
  val phonebook_v2 = linearPhonebook.toMap // only for linear collections of 2 element tuples

  // map -> linear collection
  val linearPhonebook_v2 = phonebook.toList // toSeq, toVector, toArray, toSet

  // map, flatMap, filter
  val aProcessedPhonebok = phonebook.map(pair => (pair._1.toUpperCase, pair._2))
  ...
  // mapping values
  val prefixNumbers = phonebook.view.mapValues(number => s"1-800-$number").toMap
