...
object MapFlatMapFilterFor {
  ...
  // All possible combinations of all the elements of these lists, in the format "1a - black"
  ...
  */
  ...
  val combinationsFor = for {
    number <- numbers if number % 2 == 0 // generator + if guard
    char <- chars
    color <- colors
  ...
  val toPairList: Int => LList[Int] = value => Cons(value, Cons(value + 1, Empty()))
  ...
  val combinationNumbers = for {
    ...
    chr <- Cons('a', Cons('b', Cons('c', Empty())))
  } yield s"$num-$chr"
