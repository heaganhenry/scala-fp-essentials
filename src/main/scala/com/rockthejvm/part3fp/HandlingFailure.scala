...
object HandlingFailure {
  ...
  val anIncrementedTry = aTry.map(x => s"$x")
  ...
  val stringLengthPure_v2 = betterUnsafeMethod().map(_.length).getOrElse(-1)
  val aSafeChain = betterUnsafeMethod().orElse(betterBackupMethod()).map(_.length).getOrElse(-1)
  ...
  val finalHtml_v2 = Try(HttpService.getConnection(host, port)).flatMap(conn => Try(conn.get(myDesiredURL)))
  ...
  val maybeCon = Try(HttpService.getConnection(host, port))
  val maybeHtml = maybeCon.flatMap(conn => Try(conn.get(myDesiredURL)))
